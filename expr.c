...
// being the sub-tree of previous expression (or NULL) and the right-hand
// child being the next expression. Each A_GLUE node will have size filed
...
static struct ASTnode *member_access(struct ASTnode *left, int withpointer)
{
    ...
    // we get the base address, not the value at the address.
    if (!withpointer)
    {
...
static struct ASTnode *primary(void)
{
    ...
    case T_IDENT:
        ...
        // See if this identifier exist as a symbol. For arrays, set rvalue to 1.
...
struct ASTnode *prefix(void)
{
    ...
    case T_MINUS:
        ...
        // also widen this to int so that it's singed
        tree->rvalue = 1;
        if (tree->type == P_CHAR)
            tree->type = P_INT;
